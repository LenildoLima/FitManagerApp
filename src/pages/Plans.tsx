        ...
        {planos.map((p) => (
          <div key={p.id} className="rounded-xl border border-border bg-card p-5 transition-smooth hover:border-primary/40 hover:shadow-glow">
            <div className="flex items-start justify-between">
              ...
              <span className="rounded-md border border-primary/30 bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary uppercase">
                {p.duracao}
              </span>
            </div>
            <p className="mt-5 text-3xl font-bold">
              {formatarMoeda(p.valor)}
              <span className="text-xs font-normal text-muted-foreground lowercase"> /{p.duracao}</span>
            </p>
            <div className="mt-4 flex items-center justify-between border-t border-border pt-3 text-xs text-muted-foreground">
              <span>{p.acessos_por_dia} acesso(s) por dia</span>
